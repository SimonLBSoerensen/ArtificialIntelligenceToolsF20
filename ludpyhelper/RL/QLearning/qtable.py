...
class QTable:
    ...
    def append_buffer(self, current_state, action, new_state, reward, terminal_state, save_experience=True, new_state_list=False, auto_update_episode=True):
        self.buffer.append([current_state, action, new_state, reward, terminal_state, save_experience, new_state_list, auto_update_episode])

    def introduce_buffer(self, all=True):
        while len(self.buffer):
            current_state, action, new_state, reward, terminal_state, save_experience, new_state_list, auto_update_episode = self.buffer.pop(0)
            self.update(current_state, action, new_state, reward, terminal_state, save_experience, new_state_list, auto_update_episode)

            if not all:
                break

    def update(self, current_state, action, new_state, reward, terminal_state, save_experience=True, new_state_list=False, auto_update_episode=True, resize=True):
        self.update_count += 1
        ...
        if save_experience:
            self.aer.append([current_state, action, new_state, reward, terminal_state, new_state_list], td, resize)
